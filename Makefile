...
ETAGS = etags
CTAGS = ctags
DIST_SUBDIRS = doc testsuite gnu/bytecode gnu/math gnu/lists gnu/text \
	gnu/mapping gnu/xml gnu/kawa/xml gnu/expr gnu/kawa/util \
	gnu/kawa/reflect gnu/kawa/functions gnu/kawa/lispexpr \
	gnu/kawa/models gnu/kawa/swingviews gnu/kawa/swtviews kawa \
	gnu/kawa/slib gnu/commonlisp gnu/q2 gnu/ecmascript \
	gnu/kawa/servlet gnu/kawa/echo2 gnu/kawa/sax gnu/kawa/ant \
	gnu/jemacs/lang gnu/jemacs/buffer gnu/jemacs/lisp \
	gnu/jemacs/testsuite gnu/jemacs/swing gnu/jemacs/swt \
	gnu/xquery gnu/kawa/xslt gnu/brl gnu/kawa/brl gnu/kawa/android \
	bin
...
ACLOCAL = ${SHELL} /home/runner/work/ai2-kawa/ai2-kawa/missing --run aclocal-1.11
AMTAR = ${SHELL} /home/runner/work/ai2-kawa/ai2-kawa/missing --run tar
AUTOCONF = ${SHELL} /home/runner/work/ai2-kawa/ai2-kawa/missing --run autoconf
AUTOHEADER = ${SHELL} /home/runner/work/ai2-kawa/ai2-kawa/missing --run autoheader
AUTOMAKE = ${SHELL} /home/runner/work/ai2-kawa/ai2-kawa/missing --run automake-1.11
AWK = gawk
BYTECODE_VERSION = 1.11-ai2
CC = gcc
...
CXXFLAGS = -g -O2
CYGPATH_W = echo
DEFS = -DPACKAGE_NAME=\"kawa\" -DPACKAGE_TARNAME=\"kawa\" -DPACKAGE_VERSION=\"1.11-ai2\" -DPACKAGE_STRING=\"kawa\ 1.11-ai2\" -DPACKAGE_BUGREPORT=\"\" -DPACKAGE_URL=\"\" -DPACKAGE=\"kawa\" -DVERSION=\"1.11-ai2\"
DEPDIR = .deps
ECHO_C = 
ECHO_N = -n
ECHO_T = 
...
LTLIBOBJS = 
MAINT = #
MAKEINFO = ${SHELL} /home/runner/work/ai2-kawa/ai2-kawa/missing --run makeinfo
MKDIR_P = /usr/bin/mkdir -p
OBJEXT = o
PACKAGE = kawa
PACKAGE_BUGREPORT = 
PACKAGE_NAME = kawa
PACKAGE_STRING = kawa 1.11-ai2
PACKAGE_TARNAME = kawa
PACKAGE_URL = 
PACKAGE_VERSION = 1.11-ai2
PATH_SEPARATOR = :
SAX2_PREPROCESS = 
SET_MAKE = 
SHELL = /bin/bash
STRIP = 
TOP_BUILDDIR = /home/runner/work/ai2-kawa/ai2-kawa
VERSION = 1.11-ai2
XQTS_PATH = 
abs_builddir = /home/runner/work/ai2-kawa/ai2-kawa
abs_srcdir = /home/runner/work/ai2-kawa/ai2-kawa
abs_top_builddir = /home/runner/work/ai2-kawa/ai2-kawa
abs_top_srcdir = /home/runner/work/ai2-kawa/ai2-kawa
ac_ct_CC = gcc
ac_ct_CXX = g++
am__include = include
am__leading_dot = .
...
install_sh = ${SHELL} /home/runner/work/ai2-kawa/ai2-kawa/install-sh
...
kawa-1.11-ai2.jar: ./kawa-jar.stamp $(META_INF_FILES)
	$(JAR) cmf $(srcdir)/jar-manifest $@ $(META_INF_FILES)\
	    `find gnu kawa -name '*.class' -print`

kawart-1.11-ai2.jar: ./kawa-jar.stamp $(srcdir)/classes-to-skip-for-runtime
	$(JAR) cf $@ \
	  `find gnu kawa -name '*.class' -print | grep -v -f $(srcdir)/classes-to-skip-for-runtime`
...
dist-kawa-1.11-ai2.jar: $(DIST_ARCHIVES)
	srcdir=`(cd $(srcdir); /bin/pwd)` \
	  ...
	  && ./kawa-1.11-ai2/configure --with-java-source=6compat5 --with-servlet  \
	  && CLASSPATH=.:$$CLASSPATH $(MAKE) \
	  && mv kawa-1.11-ai2.jar ../dist-kawa-1.11-ai2.jar \
	  && cd .. && rm -rf tmpdir
...
bytecode-1.11-ai2.jar: ./gnu/bytecode/java-classes.stamp
	$(JAR) cmf $(srcdir)/gnu/bytecode/jar-manifest \
	  bytecode-1.11-ai2.jar \
	  `find gnu/bytecode -name '*.class' -print` 

kawa-1.11-ai2-src.zip: kawa-1.11-ai2.tar.gz
	mkdir zip-temp
	cd zip-temp ;\
	  tar xzf ../kawa-1.11-ai2.tar.gz; \
	  zip -q -r ../kawa-1.11-ai2-src.zip kawa-1.11-ai2
	rm -rf zip-temp

kawa-compiled.zip: kawa-1.11-ai2-compiled.zip
kawa.jar: kawa-1.11-ai2.jar
kawa-src.zip: kawa-1.11-ai2-src.zip
jemacs.jar: jemacs-0.11-ai2.jar
bytecode.jar: bytecode-1.11-ai2.jar
...
install-javadoc-html:
	...
	CLASSPATH=$(JAVADOC_EXTRA_PATH):$$CLASSPATH \
...
install-jar:
	...
	$(INSTALL_DATA) kawa-1.11-ai2.jar \
	  $(DESTDIR)$(javadir)/kawa-1.11-ai2.jar
	cd $(DESTDIR)$(javadir) && rm -f kawa.jar \
	  && $(LN_S) kawa-1.11-ai2.jar kawa.jar
#	$(GCJ_DBTOOL) -f $(DESTDIR)`$(GCJ_DBTOOL) -p` $(DESTDIR)$(javadir)/kawa-1.11-ai2.jar $(libdir)/libkawa-$(VERSION).so

install-bytecode-jar: bytecode-1.11-ai2.jar
	@$(NORMAL_INSTALL)
	$(mkinstalldirs) $(DESTDIR)$(javadir)
	$(INSTALL_DATA) bytecode-1.11-ai2.jar \
	  $(DESTDIR)$(javadir)/bytecode-1.11-ai2.jar
	cd $(DESTDIR)$(javadir) && rm -f bytecode.jar \
	  && $(LN_S) bytecode-1.11-ai2.jar bytecode.jar

uninstall-local: uninstall-jar
uninstall-jar:
	rm -f $(DESTDIR)$(javadir)/kawa-1.11-ai2.jar \
	  $(DESTDIR)$(javadir)/kawa.jar

uninstall-bytecode-jar:
	rm -f $(DESTDIR)$(javadir)/bytecode-1.11-ai2.jar \
...
kawa/Version.java: $(srcdir)/kawa/Version.java.in $(srcdir)/configure.in
	...
	sed -e 's|VERSION|1.11-ai2'"$$SVN_REVISION|" <$(srcdir)/kawa/Version.java.in >$@

all-recursive:  kawa/Version.java selected-java-source
all: kawa-1.11-ai2.jar kawart-1.11-ai2.jar
...
distall:  distcheck dist-kawa-1.11-ai2.jar kawa-doc-$(VERSION).tar.gz
# Consider also:
# kawa-1.11-ai2-src.zip jemacs-0.11-ai2.jar bytecode-1.11-ai2.jar
